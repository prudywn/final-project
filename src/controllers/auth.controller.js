...
export const signup = async (req, res, next) => {
  try{
    ...
    res
      .status(201)
      .json({ message: 'User registered successfully', user: {id: user.id, name: user.name, email: user.email, role: user.role } });
...
export const signin = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ message: 'User signed in successfully', user: { id: user.id, name: user.name, email: user.email, role: user.role } });
  } catch (error) {
